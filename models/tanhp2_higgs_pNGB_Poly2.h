#ifndef TANHP2_HIGGS_PNGB_POLY2_H
#define TANHP2_HIGGS_PNGB_POLY2_H
...
#define MODELNAME tanhp2_higgs_pNGB_Poly2
  ...
  public:
    ...
    auto potentialTerms(Tag<0>) // Inflaton tanh^2 potential.
    {
      return 0.5 * pow<2>(M / phii) * pow<2>(tanh(fldS(0_c) * phii / M));
    }
    ...
    {
      return (M / phii) * tanh(fldS(0_c) * phii / M) / pow<2>(cosh(fldS(0_c) * phii / M)) +
    ...
    {
      return (1.0 - 3.0 * pow<2>(tanh(fldS(0_c) * phii / M))) / pow<2>(cosh(fldS(0_c) * phii / M)) +
...
#endif // TANHP2_HIGGS_PNGB_POLY2_H
